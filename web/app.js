...
const PDFViewerApplication = {
  ...
  async _initializeL10n() {
    this.l10n = this.externalServices.createL10n(typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC') ? { locale: AppOptions.get('locale') } : null);
  ...
  async _initializeViewerComponents() {
    ...
    const findController = new PDFFindController({
      ...
      eventBus,
      updateMatchesCountOnProgress: typeof PDFJSDev === 'undefined' ? !window.isGECKOVIEW : !PDFJSDev.test('GECKOVIEW'),
    });
    this.findController = findController;

    const pdfScriptingManager = new PDFScriptingManager({
      eventBus,
      sandboxBundleSrc: typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC || CHROME') ? AppOptions.get('sandboxBundleSrc') : null,
      externalServices,
    ...
    if (appConfig.toolbar) {
      if (typeof PDFJSDev === 'undefined' ? window.isGECKOVIEW : PDFJSDev.test('GECKOVIEW')) {
        this.toolbar = new Toolbar(appConfig.toolbar, eventBus, l10n, await this._nimbusDataPromise, externalServices);
      } else {
  ...
  async close() {
    ...
    if ((typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) && this.pdfDocument?.annotationStorage.size > 0 && this._annotationStorageModified) {
      try {
